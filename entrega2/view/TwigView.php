...
class TwigView {
    ...
    public static function getTwig() {
        $loader = new Twig_Loader_Filesystem(array('./template/compra', './template/listado', './template/producto', './template/usuario', './template/venta', './template/', './template/menu/'));
        $config = array();
        if($_SERVER['HTTP_HOST'] != "localhost") {
            $path = "https://".$_SERVER['HTTP_HOST']."/entrega2"; // Esto es porque en grupo46.proyecto2016.linti.unlp.edu.ar ya apunta a entrega2
            //$config = array('cache' => '/tmp');
        }
        else {
            $path = "http://".$_SERVER['HTTP_HOST']."/grupo46/entrega2"; // Esto es porque nuestro localhost es localhost/grupo46/
        }

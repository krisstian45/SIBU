...
class Sesion extends Base {
	...
	public function __construct() {
        parent::__construct();
        //Agrego esta linea por si por url quiere ir a http://localhost/grupo46/entrega3/index.php/?action=Sesion&request=login
        ...
        try {
            if(isset($_POST["usuario"]) && isset($_POST["password"]) && isset($_POST["token"]) ) {
                ...
                if(! $token || $token != $_POST['token'] )
                    throw new Exception("Error origen del formulario");
                unset($_POST['token']);

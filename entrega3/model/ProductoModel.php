...
class ProductoModel extends PDORepository  {
    ...
    public function agrearMenu($data)//en proceso de ver que onda..x
    {
        $conn = $this->getConnection();
        $stmt = $conn->prepare("UPDATE producto
            SET
        ...
        $stmt->bindValue(":descripcion", $data['descripcion'], PDO::PARAM_STR);
        $stmt->bindValue(":categoria_id", $data['categoria_id'], PDO::PARAM_STR);
        $stmt->bindValue(":en_menu", 1,PDO::PARAM_STR);
        $stmt->execute();
        $this->closeConnection($conn);
    }
    ...
    {
        $data= (array)$id;
        $stmt = $this->query("SELECT producto.precio_venta_unitario FROM producto WHERE id = ?", $data);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    // public function obtenerProductosEnMenu($fecha)
    // {
    //     # code...
    //     $data= (array)$fecha;
    //     $stmt = $this->query("SELECT * FROM producto LEFT JOIN menu on (id = producto_id) WHERE (menu.fecha = ?)", $data);
